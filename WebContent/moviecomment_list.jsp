...
<jsp:include page="frame/navigation_bar_frame.jsp" flush="true" />
   ...
   <div style="padding: 100px; background-color: #fff;">
			...
			<div style="text-align:center; padding:20px">	<h1><a href="movieinfo-show.do?id=<s:property value="#request.MovieInfoid"/>"><s:property value="#request.MovieInfoname"/></a></h1>
			</div>
					...
					<form action="moviecomment-save.do"  onsubmit="return islogin()" method="post">
                   	<table id="pinglun" class="table1">
							<tr>
								...
								<input  id="infoid"  name="infoid" type="hidden" value="<s:property value="#request.infoid"/>"/>
                         	...
                         	<br>
                         	<br>
    <s:if test="#request.MovieComment == null || #request.MovieComment.size() == 0">
       暂无评论
	</s:if>
	<s:else>
	 ...
	 <br>
			...
			<s:iterator value="#request.MovieComment">
			 <hr>
				    <div class="col-sm-6" style="text-align:left">
					${username} <label class="label label-success">${score}</label>
					</div>
				    ...
				    <br>
				     ...
				     <a href="moviecomment-check.do?MovieComment_id=${comment_id}&page=${page}&infoid=${MovieInfoid}">举报</a>     
				    </div>
...
<nav aria-label="Page navigation">
  ...
  <li><a href="moviecomment-ListByPage.do?infoid=<s:property value="#request.MovieInfoid"/>&page=1">首页</a></li>
    <li>
      <a href="moviecomment-ListByPage.do?infoid=<s:property value="#request.MovieInfoid"/>&page=<%
      if(pageNo>1)
      {out.print(pageNo-1);
      }
      else if(pageNo<=1)
     ...
     <% for(int i=1;i<=rpage;i++)
        {
          %>
           <li><a href="moviecomment-ListByPage.do?infoid=<s:property value="#request.MovieInfoid"/>&page=<%=i%>">
           <%=i%>
           </a></li>
          <% 
        }
  ...
  <!--   <li><a href="moviecomment-ListByPage.do?infoid=${id}&page<s:property value="#request.rpage"/>">末页</a></li> --> 
   <li><a href="moviecomment-ListByPage.do?infoid=<s:property value="#request.MovieInfoid"/>&page=<%=rpage%>">末页</a></li>
  </ul>
</nav>
